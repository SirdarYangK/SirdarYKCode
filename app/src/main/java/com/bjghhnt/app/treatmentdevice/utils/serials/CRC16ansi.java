package com.bjghhnt.app.treatmentdevice.utils.serials;

/**
 * Uses irreducible polynomial:  1 + x^2 + x^15 + x^16
 * Created by Q on 07/06/2016.
 */

public class CRC16ansi {

	private static final int[] TABLE_H = {
			
	};

	private static final int[] TABLE_L = {
			
	};

	public static String getCRC(String hexString) {
		int index;
		int CRCLow = 0xFF;
		int CRCHigh = 0xFF;
		int length = hexString.length();
		for (int k = 0; k < length; k += 2) {
			index = CRCHigh ^ Integer.parseInt(hexString.substring(k, k + 2), 16);
			// bitwise xor
			CRCHigh = CRCLow ^ TABLE_H[index];
			CRCLow = TABLE_L[index];
		}
		return String.format("%02X", (0xFF & CRCHigh)) + String.format("%02X", (0xFF & CRCLow));
	}
}
